...
// Send email to admin, if admin diesn't take action on leave application within 24
const leaveApplicationRemainder = inngest.createFunction(
  ...
  async ({event, step}) => {
    ...
    if(leaveApplication?.status === "PENDING"){
      ...
      // send remainder email to admin to taje action on leave application
      await sendEmail({
...
const attendanceRemainderCron = inngest.createFunction(
  { id: "attendance-remainder-cron", triggers: [{cron: "TZ=Asia/Kolkata 30 11 6 * * *"} ]},
   // 06:00 UTC = 11:30 AM IST
  async ({step}) => {
    // step 1: get today range (IST)
    ...
    // step 2: get all active, not deleted employees
    const activeEmployees = await step.run("get-avtive-employees", async () => {
