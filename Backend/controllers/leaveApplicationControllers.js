...
export const createLeave = async(req, res)=>{
    try {
        ...
        if(employee.isDeleted){
            return res.status(403).json({error: "Your account is deactivated. You cant apply for leaves"});
        }
        ...
        if(new Date(endDate) < new Date(startDate) ){
            return res.status(400).json({error: "End Date can't before start date"});
        }
